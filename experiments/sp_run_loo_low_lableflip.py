...
# DATASET_NAME = 'cifar'
# INITIAL_MODEL_PATH = "../result/supplementary_lowfilp_test_cifar/normal/cifar_0701_2347/CIFAR10_CNN.pkl"
# SAVE_DIR_BASE = f"../result/supplementary_loo_low_flip_test_{DATASET_NAME}/"
# done_result_file = '../result/supplementary_lowfilp_test_cifar/normal/cifar_0701_2347/result_final.json'
# price_test = False


# DATASET_NAME = 'svhn'
# INITIAL_MODEL_PATH = "../result/supplementary_lowfilp_test_svhn/normal/svhn_0702_1033/SVHN_CNN.pkl"
# SAVE_DIR_BASE = f"../result/supplementary_loo_low_flip_test_{DATASET_NAME}/"
# done_result_file = '../result/supplementary_lowfilp_test_svhn/normal/svhn_0702_1033/result_final.json'
# price_test = False

# DATASET_NAME = 'emnist'
# INITIAL_MODEL_PATH = "../result/supplementary_lowfilp_test_emnist/normal/emnist_0701_1237/EMNIST_CNN.pkl"
# SAVE_DIR_BASE = f"../result/supplementary_loo_low_flip_test_{DATASET_NAME}/"
# done_result_file = '../result/supplementary_lowfilp_test_emnist/normal/emnist_0701_1237/result_final.json'
# price_test = False

DATASET_NAME = 'fashionmnist'
INITIAL_MODEL_PATH = "../result/supplementary_lowfilp_test_fashionmnist/normal/fashionmnist_0701_1446/FashionMNIST_CNN.pkl"
SAVE_DIR_BASE = f"../result/supplementary_loo_low_flip_test_{DATASET_NAME}/"
done_result_file = '../result/supplementary_lowfilp_test_fashionmnist/normal/fashionmnist_0701_1446/result_final.json'
price_test = False

def get_experiment_data_params(group_name):
    noise_params = {
        'client_flip_ratio': [0.2, 0.2, 0.2, 0.2],
        'label_flip_ratio': [0.1, 0.2, 0.3, 0.4]
    }
...
from flex.pool.utilsSVselection import update_reputation
from flex.pool import collect_clients_weights_pt
from collections import defaultdict, deque
from flex.pool import FlexPool
from flex.pool import fed_avg
from flex.pool import set_aggregated_weights_pt
from flex.pool.utilsSVselection import calculate_loo_contribution
...
def train_n_rounds(n_rounds, clients_per_round,initial_model_path=None):
    acc_history_loo_list = []
    loo_selected_client_id_list = []
    ...
    pool_loo = FlexPool.client_server_pool(
        fed_dataset=federated_data, server_id=server_id, init_func=build_from_saved
    )

    # record shapley values for each client
    loo_record = defaultdict(list)
    for actor_id in pool_loo.clients.actor_ids:
        loo_record[actor_id]


    metrics = pool_loo.servers.map(evaluate_global_model)
    loss, acc = metrics[0]
    print(f"Server: inital Test acc: {acc:.4f}, test loss: {loss:.4f}")

    acc_history_loo_list.append(acc)

    # record the selection history of in the last 5 rounds
    selection_history_rounds = {client_id: deque(maxlen=5) for client_id in pool_loo.clients.actor_ids}

    sm = SeedManager(42)
    sm.set_global_seed()
    sm.register_algorithm("loo")

    for i in range(n_rounds):

        print(f"\nRunning round: {i + 1} of {n_rounds}")
        sm.prepare_round('loo', i)

        recent_selection_counts = {client_id: sum(selection_history_rounds[client_id]) for client_id in
                                   selection_history_rounds.keys()}
        loo_selected_pool = pool_loo.clients.select(
            clients_per_round(Reputation_dict, Bid_dict, Budget, exparam.γ, exparam.α, exparam.β, i, recent_selection_counts))
        loo_selected_clients = loo_selected_pool.clients
        loo_selected_client_id_list.append(loo_selected_clients.actor_ids)
              # Record the selection history of the selected clients
        for client_id in loo_selected_clients.actor_ids:
            selection_history_rounds[client_id].append(1)  # indicate this client has been selected
        for client_id in selection_history_rounds.keys():
            if client_id not in loo_selected_clients.actor_ids:
                selection_history_rounds[client_id].append(0)  # indicate this client has not been selected
        print(
            f"Selected clients for this round and flip ratio: { {id: nodes_label_flip_ratio[id] for id in loo_selected_clients.actor_ids} }")
        # Deploy the server model to the selected clients
        pool_loo.servers.map(deploy_server_model_pt, loo_selected_clients)
        # Each selected client trains her model
        loo_selected_clients.map(train, round=i)
        # Calculate Shapley values for each client
        loo_values = calculate_loo_contribution(loo_selected_clients, acc_history_loo_list[-1], val_dataset, device)
        # print(shapley_values)
        # Update reputation and bid for each client
        update_reputation(Reputation_dict, Bid_dict, loo_values,loo_record, exparam.ω, exparam.ψ, 5)
        for k, v in loo_values.items():
            loo_record[k].append(v)

        # The aggregador collects weights from the selected clients and aggregates them
        pool_loo.aggregators.map(collect_clients_weights_pt, loo_selected_clients)
        pool_loo.aggregators.map(fed_avg)
        # The aggregator send its aggregated weights to the server
        pool_loo.aggregators.map(set_aggregated_weights_pt, pool_loo.servers)
        metrics = pool_loo.servers.map(evaluate_global_model)
        loss, acc = metrics[0]
        ...
        if (i + 1) % 20 == 0:
            data = {
                ...
                'acc_history_loo_list': acc_history_loo_list,
                'loo_selected_client_id_list': loo_selected_client_id_list,
                'loo_record': loo_record,
            }
            x_axis = range(len(acc_history_loo_list))
            ...
            plt.plot(x_axis, acc_history_loo_list, label='SBRO-FL-loo')
