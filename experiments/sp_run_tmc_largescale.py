...
import numpy as np
import random
from flex.pool import select_nodes_after
from flex.common import save_flex_model
import torch.nn as nn
from utils import SeedManager

# Set Parameters
dataset_name = 'cifar'  # Options: 'emnist', 'cifar', 'fashionmnist', 'svhn'
price_test = False
save_dir = f"../result/supplementary_tmc_largescale_test_{dataset_name}/"

def get_experiment_data_params(group_name):
    noise_params = {
        'client_flip_ratio': [0.2, 0.2, 0.2, 0.2],
        'label_flip_ratio': [0.2, 0.4, 0.6, 0.8]
    }
...
from flex.pool.utilsSVselection import calculate_gtg_shapley
...
def train_n_rounds(n_rounds, clients_per_round):
    ...
    sm = SeedManager(42)
    sm.set_global_seed()
    sm.register_algorithm("SBRO")
    sm.register_algorithm("RS")
    sm.register_algorithm("All")
    sm.register_algorithm("HQRS")
    ...
    for i in range(n_rounds):
        ...
        shapley_values = calculate_gtg_shapley(SBRO_selected_clients, acc_history_SBRO_list[-1], val_dataset, device)
        ...
        loss, acc = metrics[0]
        print(f"Server: Random Selection Test acc: {acc:.4f}, test loss: {loss:.4f}")
        acc_history_RS_list.append(acc)

        # Select all clients
        sm.prepare_round('All', i)
        ...
        loss, acc = metrics[0]
        print(f"All: Test acc: {acc:.4f}, test loss: {loss:.4f}")
        acc_history_All_list.append(acc)


        # High-Quality Random Selection (HQRS)
        sm.prepare_round('HQRS', i)
        ...
        loss, acc = metrics[0]
        print(f"HQRS: Test acc: {acc:.4f}, test loss: {loss:.4f}")
        acc_history_HQRS_list.append(acc)
        print('-------------------------------------------------------------------------')
    ...
    with open(os.path.join(file_path, 'result_final.json'), 'w') as f:
        yaml.dump(data, f)
train_n_rounds(exparam.n_rounds, clients_per_round=select_nodes_after)
