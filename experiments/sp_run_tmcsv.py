...
# SAVE_DIR_BASE = f"../result/supplementary_tmc_test_{DATASET_NAME}/"
...
from flex.pool.utilsSVselection import calculate_tmc_shapley
...
def train_n_rounds(n_rounds, clients_per_round,initial_model_path=None):
    acc_history_tmc_list = []
    tmc_selected_client_id_list = []
    ...
    pool_tmc = FlexPool.client_server_pool(
        fed_dataset=federated_data, server_id=server_id, init_func=build_from_saved
    )

    # record shapley values for each client
    tmc_record = defaultdict(list)
    for actor_id in pool_tmc.clients.actor_ids:
        tmc_record[actor_id]


    metrics = pool_tmc.servers.map(evaluate_global_model)
    loss, acc = metrics[0]
    print(f"Server: inital Test acc: {acc:.4f}, test loss: {loss:.4f}")

    acc_history_tmc_list.append(acc)

    # record the selection history of in the last 5 rounds
    selection_history_rounds = {client_id: deque(maxlen=5) for client_id in pool_tmc.clients.actor_ids}

    sm = SeedManager(42)
    sm.set_global_seed()
    sm.register_algorithm("tmc")

    for i in range(n_rounds):

        print(f"\nRunning round: {i + 1} of {n_rounds}")
        sm.prepare_round('tmc', i)

        recent_selection_counts = {client_id: sum(selection_history_rounds[client_id]) for client_id in
                                   selection_history_rounds.keys()}
        tmc_selected_pool = pool_tmc.clients.select(
            clients_per_round(Reputation_dict, Bid_dict, Budget, exparam.γ, exparam.α, exparam.β, i, recent_selection_counts))
        tmc_selected_clients = tmc_selected_pool.clients
        tmc_selected_client_id_list.append(tmc_selected_clients.actor_ids)
              # Record the selection history of the selected clients
        for client_id in tmc_selected_clients.actor_ids:
        ...
        for client_id in selection_history_rounds.keys():
            if client_id not in tmc_selected_clients.actor_ids:
                selection_history_rounds[client_id].append(0)  # indicate this client has not been selected
        print(
            f"Selected clients for this round and flip ratio: { {id: nodes_label_flip_ratio[id] for id in tmc_selected_clients.actor_ids} }")
        # Deploy the server model to the selected clients
        pool_tmc.servers.map(deploy_server_model_pt, tmc_selected_clients)
        # Each selected client trains her model
        tmc_selected_clients.map(train, round=i)
        # Calculate Shapley values for each client
        tmc_values = calculate_tmc_shapley(tmc_selected_clients, acc_history_tmc_list[-1], val_dataset, device)
        # print(shapley_values)
        # Update reputation and bid for each client
        update_reputation(Reputation_dict, Bid_dict, tmc_values,tmc_record, exparam.ω, exparam.ψ, 5)
        for k, v in tmc_values.items():
            tmc_record[k].append(v)

        # The aggregador collects weights from the selected clients and aggregates them
        pool_tmc.aggregators.map(collect_clients_weights_pt, tmc_selected_clients)
        pool_tmc.aggregators.map(fed_avg)
        # The aggregator send its aggregated weights to the server
        pool_tmc.aggregators.map(set_aggregated_weights_pt, pool_tmc.servers)
        metrics = pool_tmc.servers.map(evaluate_global_model)
        loss, acc = metrics[0]
        ...
        if (i + 1) % 20 == 0:
            data = {
                ...
                'acc_history_tmc_list': acc_history_tmc_list,
                'tmc_selected_client_id_list': tmc_selected_client_id_list,
                'tmc_record': tmc_record,
            }
            x_axis = range(len(acc_history_tmc_list))
            ...
            plt.plot(x_axis, acc_history_tmc_list, label='SBRO-FL-tmc')
