...
from flex.pool import select_nodes_linear_after
...
# SAVE_DIR_BASE = f"../result/supplementary_afterlinear_test_{DATASET_NAME}/"
...
def train_n_rounds(n_rounds, clients_per_round,initial_model_path=None):
    acc_history_linear_list = []
    linear_selected_client_id_list = []
    ...
    pool_linear = FlexPool.client_server_pool(
    ...
    for actor_id in pool_linear.clients.actor_ids:
        sv_record[actor_id]


    metrics = pool_linear.servers.map(evaluate_global_model)
    loss, acc = metrics[0]
    print(f"Server: inital Test acc: {acc:.4f}, test loss: {loss:.4f}")

    acc_history_linear_list.append(acc)

    # record the selection history of in the last 5 rounds
    selection_history_rounds = {client_id: deque(maxlen=5) for client_id in pool_linear.clients.actor_ids}

    sm = SeedManager(42)
    sm.set_global_seed()
    sm.register_algorithm("linear")

    for i in range(n_rounds):

        print(f"\nRunning round: {i + 1} of {n_rounds}")
        sm.prepare_round('linear', i)

        recent_selection_counts = {client_id: sum(selection_history_rounds[client_id]) for client_id in
                                   selection_history_rounds.keys()}
        linear_selected_pool = pool_linear.clients.select(
            clients_per_round(Reputation_dict, Bid_dict, Budget, recent_selection_counts))
        linear_selected_clients = linear_selected_pool.clients
        linear_selected_client_id_list.append(linear_selected_clients.actor_ids)
              # Record the selection history of the selected clients
        for client_id in linear_selected_clients.actor_ids:
        ...
        for client_id in selection_history_rounds.keys():
            if client_id not in linear_selected_clients.actor_ids:
                selection_history_rounds[client_id].append(0)  # indicate this client has not been selected
        print(
            f"Selected clients for this round and flip ratio: { {id: nodes_label_flip_ratio[id] for id in linear_selected_clients.actor_ids} }")
        # Deploy the server model to the selected clients
        pool_linear.servers.map(deploy_server_model_pt, linear_selected_clients)
        # Each selected client trains her model
        linear_selected_clients.map(train, round=i)
        # Calculate Shapley values for each client
        shapley_values = exact_shapley_value(linear_selected_clients, acc_history_linear_list[-1], val_dataset, device)
        ...
        pool_linear.aggregators.map(collect_clients_weights_pt, linear_selected_clients)
        pool_linear.aggregators.map(fed_avg)
        # The aggregator send its aggregated weights to the server
        pool_linear.aggregators.map(set_aggregated_weights_pt, pool_linear.servers)
        metrics = pool_linear.servers.map(evaluate_global_model)
        loss, acc = metrics[0]
        ...
        if (i + 1) % 20 == 0:
            data = {
                ...
                'acc_history_linear_list': acc_history_linear_list,
                'linear_selected_client_id_list': linear_selected_client_id_list,
                'sv_record': sv_record,
            }
            x_axis = range(len(acc_history_linear_list))
            ...
            plt.plot(x_axis, acc_history_linear_list, label='SBRO-FL-linear')
...
train_n_rounds(exparam.n_rounds, clients_per_round=select_nodes_linear_after, initial_model_path=INITIAL_MODEL_PATH)
