...
# SAVE_DIR_BASE = f"../result/supplementary_revisedselect_{DATASET_NAME}/"
...
Reputationh_dict = {node_id: [0] for node_id in federated_data.data.keys()}
...
def train_n_rounds(n_rounds, clients_per_round,initial_model_path=None):
    ...
    selection_history_rounds = {client_id: deque(maxlen=1) for client_id in pool_SBRO.clients.actor_ids}
    ...
    for i in range(n_rounds):
        ...
        sbro_training(selection_history_rounds, pool_SBRO, Reputationh_dict, acc_history_SBRO_list, sv_record, SBRO_selected_client_id_list, 5)

        if (i + 1) % 20 == 0:
            data = {
                'Bid_dict': Bid_dict,
                'Reputationh_dict': Reputationh_dict,
                'nodes_label_flip_ratio': nodes_label_flip_ratio,
                'experiment_parameters': experiment_parameters,
                'acc_history_SBROh1_list': acc_history_SBRO_list,
...
train_n_rounds(exparam.n_rounds, clients_per_round=select_nodes_after, initial_model_path=INITIAL_MODEL_PATH)
